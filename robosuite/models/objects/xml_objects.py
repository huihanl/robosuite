...
class BiPartObject(MujocoXMLObject):
    """
    Door with handle (used in Door)
    ...
    Args:
        ...
        lock (bool): Whether to use the locked door variation object or not
    """
    def __init__(self, xml_path, name, friction=None, damping=None):
        super().__init__(xml_path_completion(xml_path),
                         name=name, joints=None, obj_type="all", duplicate_collision_geoms=False)

        # Set relevant body names
        self.static = self.naming_prefix + "static"
        self.mobile = self.naming_prefix + "mobile"
        self.hinge_joint = self.naming_prefix + "hinge"
